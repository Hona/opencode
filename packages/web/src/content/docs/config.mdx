...
- `enabled` - Enable or disable telemetry (default: `true` when object is provided).
- `endpoint` - OTLP gRPC endpoint URL (default: `http://localhost:4317`).

You can also enable telemetry using the standard `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable. The environment variable takes precedence over config file settings.
