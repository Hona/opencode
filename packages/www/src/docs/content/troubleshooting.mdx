...
On Windows, signal the process's native auto-reset event from PowerShell. Replace `12345` with the server PID:

```powershell
pwsh -NoProfile -Command '[Threading.EventWaitHandle]::OpenExisting("Local\opencode-heap-12345").Set()'
```

This starts a short-lived PowerShell process. Windows closes its event handle automatically when that process exits.
