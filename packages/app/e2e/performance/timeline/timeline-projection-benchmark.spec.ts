...
import { buildInitialStreamEvent, buildStreamDeltaEvents, setupTimelineBenchmark, textPartID } from "./session-timeline-benchmark.fixture"
...
for (const scenario of [
  ...
  benchmark(`text projection ${scenario.historyTurns} ${scenario.historyShape}`, async ({ page, report }) => {
    ...
    const source = createServer((request, response) => {
      ...
      response.write(`data: ${JSON.stringify({ id: "evt_projection_connected", type: "server.connected", data: {} })}\n\n`)
    ...
    const send = (events: OpenCodeEvent[]) => responses.forEach((response) => events.forEach((event) => response.write(`data: ${JSON.stringify(event)}\n\n`)))
    try {
      await page.addInitScript(({ url, counters }) => {
        ...
        window.fetch = (input, init) => {
          ...
          return fetch(new URL(request.url).pathname === "/api/event" ? new Request(url, { signal: request.signal }) : request)
        }
      ...
      await page.evaluate(({ partID, counters }) => {
        ...
        if (counters) {
          if (!window.__timelineProjectionProbe?.calls) throw new Error("Projection instrumentation did not observe initial construction")
          window.__timelineProjectionProbe = { calls: 0, entries: 0, ms: 0 }
        }
        const measurement: Measurement = { frames: [], started: performance.now(), ready: 0, rowReplacements: 0, stop: () => {} }
        ...
        const sample = (now: number) => {
          ...
          if (next && next !== current) { measurement.rowReplacements++; current = next }
      ...
      const metrics = await page.evaluate(() => {
        ...
        data.stop()
        return { frames: data.frames, readyMs: data.ready, rowReplacements: data.rowReplacements, projection: window.__timelineProjectionProbe ?? null }
      })
      expect(emitted).toHaveLength(160)
      expect(metrics.frames.length).toBeGreaterThan(0)
      await benchmarkDiagnostics(page).stop()
      report({ ...metrics, initialReadyMs, emitted }, { ...scenario, ...fixture.workload, deltaBytes: Buffer.byteLength(JSON.stringify(deltas)), counters: process.env.PROJECTION_COUNTERS === "1", intervalMs: 25, deltas: 160, viewport: "1366x768", revision: process.env.PROJECTION_REVISION })
      if (process.env.PROJECTION_SCREENSHOT) await page.screenshot({ path: `${process.env.PROJECTION_SCREENSHOT}-${scenario.historyTurns}-${scenario.historyShape}.png` })
    } finally {
      clearInterval(timer)
