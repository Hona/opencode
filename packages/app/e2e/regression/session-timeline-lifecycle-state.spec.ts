...
test("shows and expands a running shell command without shimmering it", async ({ page }) => {
  ...
  await expect(tool.locator('[data-component="text-shimmer-v2"]')).toHaveAttribute("data-active", "true")
  await expect(tool.locator('[data-component="shell-submessage"]')).toHaveText(command)
  await expect(tool.locator('[data-component="shell-submessage"] [data-component="text-shimmer-v2"]')).toHaveCount(0)
