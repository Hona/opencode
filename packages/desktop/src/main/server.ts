...
type SpawnLocalServerOptions = {
  ...
  onBrowserRequest?: (request: BrowserControl.Request, abort: AbortSignal) => Promise<BrowserControl.Result>
}
...
function browserError(requestID: string, code: BrowserControl.ErrorCode, message: string, retryable: boolean) {
  return {
    type: "desktop.browser.response",
    version: BrowserControl.VERSION,
    requestID,
...
function browserErrorDetail(error: unknown) {
  if (error instanceof Error && "code" in error && typeof error.code === "string") {
    const code = BrowserControl.ERROR_CODES.find((item) => item === error.code) ?? "internal"
