...
export namespace Pty {
  ...
  const pty = lazy(async () => {
    log.info("pty init", {
      ...
      existingBunPtyLib: process.env.BUN_PTY_LIB,
    ...
    if (!Installation.isLocal() && !process.env.BUN_PTY_LIB) {
      ...
      log.info("setting pty lib path", { libPath })
  ...
  export async function create(input: CreateInput) {
    ...
    log.info("spawning pty", { command: normalizedCommand, args, cwd, BUN_PTY_LIB: process.env.BUN_PTY_LIB })
    try {
