...
describe("BrowserControlProtocol", () => {
  test("decodes text and Bun-compatible UTF-8 byte messages", async () => {
    const message = '{"type":"browser.control.sync","revision":1,"registrations":[]}'
  ...
  test("round trips both control directions", async () => {
    const desktop = { type: "browser.control.sync" as const, revision: 1, registrations: [] }
    ...
    const reveal = {
      type: "browser.control.reveal" as const,
      requestID: BrowserControl.RequestID.make("brr_reveal"),
      sessionID: SessionID.make("ses_reveal"),
    }
    const revealed = {
      type: "browser.control.revealed" as const,
      requestID: BrowserControl.RequestID.make("brr_reveal"),
      outcome: { type: "success" as const },
    }
    expect(
      await Effect.runPromise(BrowserControlProtocol.decodeFromServer(BrowserControlProtocol.encodeFromServer(reveal))),
    ).toEqual(reveal)
    expect(
      await Effect.runPromise(
        BrowserControlProtocol.decodeFromDesktop(BrowserControlProtocol.encodeFromDesktop(revealed)),
      ),
    ).toEqual(revealed)
  })
    ...
    expect(
      await Effect.runPromise(
        BrowserControlProtocol.decodeFromDesktop(
          '{"type":"browser.control.sync","revision":1,"registrations":[],"extra":true}',
        ).pipe(Effect.result),
      ),
